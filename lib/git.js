const util = require('./util'),
  ...
  repoPathParse = require('parse-github-repo-url'),
...
function parseRepo(remoteUrl) {
  const result = repoPathParse(remoteUrl);
  const owner = result[0];
  const project = result[1];
  const branch = result[2];
  const repo = {owner, project, repository: owner + '/' + project};
  return repo;
}
...
function release(options, remoteUrl, tagName) {
  ...
  log.execution('node-github releases#createRelease (start) - test', repo.project);
...
function uploadAssets(options, remoteUrl, assets) {
  ...
  log.execution('node-github releases#uploadAsset (start) - test');
